...
// WithMergeRange is an optional repository capability for atomically merging
// one fully aligned linear range of changes.
type WithMergeRange interface {
	Repository

	// MergeRange verifies that every change still has the supplied remote base,
	// head, and head commit, then starts one merge of the complete path.
	// The returned operation is nil when the forge has already accepted the
	// merge request;
	// callers must still observe change state to determine final completion.
	// If MergeRange returns [ErrUnsupported], it has not started a merge and the
	// caller may retry the changes with [Repository.MergeChange].
	MergeRange(context.Context, MergeRangeRequest) (MergeOperation, error)
}
