...
var _ forge.WithMergeRange = (*Repository)(nil)

// MergeRange verifies the requested linear path and starts an asynchronous
// GitHub merge of its highest pull request.
// When the pull requests belong to one stack,
// GitHub merges the open stack prefix ending at that pull request.
// A one-change range does not need to belong to a native stack.
func (r *Repository) MergeRange(
	ctx context.Context,
...
func (m *githubMergeRange) validateNativeStack() error {
	...
	if len(stack.OpenPullRequests) < len(numbers) || !slices.Equal(
		stack.OpenPullRequests[:len(numbers)],
		numbers,
	) {
		return fmt.Errorf(
			...
			stack.Number,
			formatPullRequestNumbers(stack.OpenPullRequests),
		)
	}
	return nil
}
